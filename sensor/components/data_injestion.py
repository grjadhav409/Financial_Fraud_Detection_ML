from sensor.logger import logging
from sensor.exception import SensorException
from sensor.entity.config_entity import DataIngestionConfig
from sensor.entity.artifact_entity import DataInjestionArtifact
from sensor.data_access.sensor_data import SensorData
import pandas as pd
from sklearn.model_selection import train_test_split

import os
import sys



class DataInjestion:

    def __init__(self, data_injestion_config:DataIngestionConfig):
        try:
            self.data_injestion_config = data_injestion_config

        except Exception as e:
            raise SensorException(e,sys)
        
    def export_data_to_feature_store(self) -> pd.DataFrame:
        """Export mongodb  data to feature store

        Raises:
            SensorException: _description_

        Returns:
            pd.DataFrame: _description_
        """
        try:
            logging.info("start: export_data_to_feature_store")
            sensor_data = SensorData()
            df = sensor_data.export_collection_as_dataframe(collection_name=self.data_injestion_config.collection_name)
            # save df into frature_store/fraud.csv
            feature_store_file_path = self.data_injestion_config.feature_store_file_path
            dir_path = os.path.dirname(feature_store_file_path)
            os.makedirs(dir_path,exist_ok=True)
            df.to_csv(feature_store_file_path,index=False,header=True)
            logging.info("done: export_data_to_feature_store")  
            return df 

        except Exception as e:
            raise SensorException(e,sys)
        
    def split_data_as_train_test(self, dataframe: pd.DataFrame) -> None:
        """
        Feature store dataset will be split into train and test file
        """

        try:
            train_set, test_set = train_test_split(
                dataframe, test_size=self.data_injestion_config.train_test_split_ratio
            )

            logging.info("Performed train test split on the dataframe")

            logging.info(
                "Exited split_data_as_train_test method of Data_Ingestion class"
            )

            dir_path = os.path.dirname(self.data_injestion_config.training_file_path)

            os.makedirs(dir_path, exist_ok=True)

            logging.info(f"Exporting train and test file path.")

            train_set.to_csv(
                self.data_injestion_config.training_file_path, index=False, header=True
            )

            test_set.to_csv(
                self.data_injestion_config.testing_file_path, index=False, header=True
            )

            logging.info(f"Exported train and test file path.")
        except Exception as e:
            raise SensorData(e,sys)
   
    def initiate_data_injestion(self)-> DataInjestionArtifact:
        try:
            dataframe = self.export_data_to_feature_store()
            self.split_data_as_train_test(dataframe=dataframe)
            data_injestion_artifact = DataInjestionArtifact(train_file_path=self.data_injestion_config.training_file_path, 
                                  test_file_path=self.data_injestion_config.testing_file_path)

            return data_injestion_artifact

        except Exception as e:
            raise SensorException(e,sys)